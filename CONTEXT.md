...
1. **Phase 1: Selection & Viability.** Validating the niche against Avatar Adjacency, Scale vs. Frequency, and Multi-Source Data Slicing potential. Executing AnswerThePublic keyword crawls and compiling the Search Intent Map.
...
3. **Phase 3: Engineering Pipeline.** Deploying scrapers to harvest registries (from multiple data feeds), cleaning/geocoding data with local cache, running Gemini enrichment loops, and compiling a local SQLite database.
4. **Phase 4: Antigravity Deployment.** Spinning up Astro/Next.js frontends, ensuring Day 1 Email Capture, Voice Schema injection, robust internal linking, and `llms.txt` generation. Deploying instantly to Railway.
